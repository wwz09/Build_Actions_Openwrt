...
sed -i "s/hostname='.*'/hostname='GDOCK'/g" package/base-files/luci2/bin/config_generate
...
#修改默认无线名称
sed -i 's/LEDE/GDOCKB-2.4G/g' package/kernel/mac80211/files/lib/wifi/mac80211.sh
...
# 修改输出文件名
# sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=GDOCK-KKT/g' include/image.mk
